...
date
args="-s 15 --duty-cycle 4.0"
# sudo time ./pmemtrace -s 15 --duty-cycle 4.0 randread sudo bash -c "head -c 1000M </dev/urandom >/mnt/pmem_emul/some_rand5.txt

sudo env "PATH=$PATH" pmemtrace $args fio-randrw-0 sudo fio --name=fio-randrw-0 --directory=/mnt/pmem_emul --size=2Gb --rw=randrw --bs=1K --direct=0 --rwmixread=0 --rwmixwrite=100 --numjobs=1 --ioengine=sync
