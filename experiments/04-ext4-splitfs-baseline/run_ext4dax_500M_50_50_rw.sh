...
date
args="-s 30 --duty-cycle 0.99" # old --duty-cycle 0.95
# sudo time ./pmemtrace -s 15 --duty-cycle 4.0 randread sudo bash -c "head -c 1000M </dev/urandom >/mnt/pmem_emul/some_rand5.txt

sudo env "PATH=$PATH" pmemtrace $args ext4-dax-fio-randrw-50 sudo fio --name=fio-randrw-0 --directory=/mnt/pmem_emul --size=500M --rw=randrw --bs=1K --direct=0 --rwmixread=50 --rwmixwrite=50 --numjobs=1 --ioengine=sync
