...
int ext4_meta_collapse_range(struct inode *inode, loff_t offset, loff_t len, handle_t *handle)
{
	 ...
	 */
	if (!ext4_test_inode_flag(inode, EXT4_INODE_EXTENTS)) {
		//printk(KERN_INFO "%s: inode doesn't use extents\n", __func__);
		return -EOPNOTSUPP;
	}
	...
	if (offset & (EXT4_CLUSTER_SIZE(sb) - 1) ||
		...
		//printk(KERN_INFO "%s: collapse range is not on block size alignments\n", __func__);
		return -EINVAL;
	}
	...
	if (offset + len >= i_size_read(inode)) {
		ret = -EINVAL;
		//printk(KERN_INFO "%s: offset + len > size of file\n", __func__);
		goto out_mutex;
	}

	/* Currently just for extent based files */
	if (!ext4_test_inode_flag(inode, EXT4_INODE_EXTENTS)) {
		//printk(KERN_INFO "%s: inode not using extents\n", __func__);
		ret = -EOPNOTSUPP;
		goto out_mutex;
	}

	//printk(KERN_INFO "%s: all checks successful\n", __func__);
	...
	if (ret) {
		//printk(KERN_INFO "%s: filemap failed\n", __func__);
		goto out_mmap;
	}
	/*
	...
	ret = filemap_write_and_wait_range(inode->i_mapping, offset + len,
					   LLONG_MAX);
	if (ret) {
		//printk(KERN_INFO "%s: filemap write and wait failed\n", __func__);
		goto out_mmap;
	}
	...
	if (ret) {
		//printk(KERN_INFO "%s: ext remove extent failed\n", __func__);
		goto out_stop;
	}

	//printk(KERN_INFO "%s: Removed extent successfully\n", __func__);
	ret = ext4_ext_remove_space(inode, punch_start, punch_stop - 1);
	if (ret) {
		//printk(KERN_INFO "%s: ext remove space failed\n", __func__);
		goto out_stop;
	}
	ext4_discard_preallocations(inode);
	...
	if (ret) {
		//printk(KERN_INFO "%s: ext shift extents failed\n", __func__);
		goto out_stop;
	}
	...
	//printk(KERN_INFO "%s: Truncated to disk size = %lu, inode size = %lu\n", __func__, EXT4_I(inode)->i_disksize, inode->i_size);
...
/**
 * Rohan making Ledger changes - Donate extent from donor inode to 
 * receiver inode. 
			   ...
			   ext4_lblk_t count, int *erp)
{
	...
	// struct timespec remove_es_time, swap_while_one_time, swap_while_two_time, swap_while_three_time, inside_swap_fn_time, swap_while_three_try_merge_time, swap_while_three_force_split_time, swap_while_three_meta_swap_time;
	
	int replaced_count = 0;

	// LEDGER_START_TIMER(inside_swap_fn_t, inside_swap_fn_time);
	...
	//LEDGER_START_TIMER(remove_es_t, remove_es_time);
	
	*erp = ext4_es_remove_extent(receiver_inode, rec_lblk, count);
	if (unlikely(*erp)) {
		//LEDGER_END_TIMER(remove_es_t, remove_es_time);
		//LEDGER_END_TIMER(inside_swap_fn_t, inside_swap_fn_time);
		return 0;
	}
	...
	*erp = ext4_es_remove_extent(donor_inode, donor_lblk, count);
	...
	while (count) {
		...
		//LEDGER_START_TIMER(swap_while_one_t, swap_while_one_time);
		...
		if (IS_ERR(receiver_path)) {
			*erp = PTR_ERR(receiver_path);
			receiver_path = NULL;
			//LEDGER_END_TIMER(swap_while_one_t, swap_while_one_time);
		finish:
			count = 0;
			goto repeat;
		}
		...
		//LEDGER_END_TIMER(swap_while_one_t, swap_while_one_time);
	        ...
	        //LEDGER_START_TIMER(swap_while_two_t, swap_while_two_time);
		...
		//LEDGER_START_TIMER(swap_while_three_t, swap_while_three_time);

		//LEDGER_START_TIMER(swap_while_three_force_split_t, swap_while_three_force_split_time);
		...
		if (len != er_len) {
			...
			*erp = ext4_force_split_extent_at(handle, receiver_inode, &receiver_path, rec_lblk + len, 0);
			if (unlikely(*erp)) {
				//LEDGER_END_TIMER(swap_while_three_force_split_t, swap_while_three_force_split_time);
				//LEDGER_END_TIMER(swap_while_three_t, swap_while_three_time);
				goto finish;
			}
		}
		if (len != ed_len) {
		...
		//LEDGER_START_TIMER(swap_while_three_meta_swap_t, swap_while_three_meta_swap_time);
		
		*erp = ext4_ext_get_access(handle, receiver_inode, receiver_path + receiver_path->p_depth);
		if (unlikely(*erp)) {
			//LEDGER_END_TIMER(swap_while_three_meta_swap_t, swap_while_three_meta_swap_time);
			//LEDGER_END_TIMER(swap_while_three_t, swap_while_three_time);
			goto finish;
		}
		*erp = ext4_ext_get_access(handle, donor_inode, donor_path + donor_path->p_depth);
		...
		//ext4_ext_mark_unwritten(donor_ex);
		//ext4_ext_mark_unwritten(rec_ex);
		
		//LEDGER_END_TIMER(swap_while_three_meta_swap_t, swap_while_three_meta_swap_time);
		
		//LEDGER_START_TIMER(swap_while_three_try_merge_t, swap_while_three_try_merge_time);
		...
		*erp = ext4_ext_dirty(handle, donor_inode, donor_path +
				      donor_path->p_depth);
		if (unlikely(*erp)) {
			//LEDGER_END_TIMER(swap_while_three_try_merge_t, swap_while_three_try_merge_time);			
			//LEDGER_END_TIMER(swap_while_three_t, swap_while_three_time);
			goto finish;
		}
		...
		*erp = ext4_ext_dirty(handle, receiver_inode,
		...
		//printk(KERN_INFO "%s: Done Swapping\n", __func__);
