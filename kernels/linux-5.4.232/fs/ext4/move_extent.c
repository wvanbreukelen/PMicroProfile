 ...
 */
static int
move_meta_extent_per_page(handle_t *handle, struct file *o_filp,
			  struct inode *donor_inode,
			  ...
			  int block_len_in_page, int *err)
{
	...
	// struct timespec swap_extents_time;
 ...
 again:
	*err = 0;
	...
	/*
	*err = mext_page_double_lock(rec_inode, donor_inode,
				     rec_page_offset, donor_page_offset,
				     pagep);
	if (unlikely(*err < 0)) {
		printk(KERN_INFO "%s: error value = %d. Err = %s\n", __func__, *err, strerror(*err));
		goto stop_journal;
	}
	*/
	...
	/* If any of extents in range became 
	 * initialized we have to
	 * fallback 
	 */

	/*
	unwritten = mext_check_coverage(rec_inode, rec_blk_offset,
					block_len_in_page, 1, err);
	if (*err)
		goto drop_data_sem;
	unwritten &= mext_check_coverage(donor_inode, donor_blk_offset,
	...
	*/
	...
	//printk(KERN_INFO "%s: calling ext4_meta_swap_extents\n", __func__);
	//LEDGER_START_TIMER(swap_extents_t, swap_extents_time);
	...
	//LEDGER_END_TIMER(swap_extents_t, swap_extents_time);
		   ...
		   loff_t count)
{
	...
	// struct timespec allocation_time, meta_extent_per_page_time;

	//printk(KERN_INFO "%s: offset1 = %lld, offset2 = %lld, count = %lld\n", __func__, offset1, offset2, count);
	//LEDGER_INIT_TIMES();	
	...
	//LEDGER_START_TIMER(allocation_t, allocation_time);
	
	ext4_fallocate_for_dr(handle, file1, 0, offset1, count);

	//LEDGER_END_TIMER(allocation_t, allocation_time);
	...
	//printk(KERN_INFO "%s: calling check arguments with rec_blk = %u, donor_blk = %u, len = %llu\n", __func__, rec_blk, donor_blk, len);
	ret = mext_check_arguments(rec_inode, donor_inode, rec_blk,
				   donor_blk, &len);
	if (ret) {
		//printk(KERN_INFO "%s: mext_check_arguments failed.\n", __func__);
		goto out;
	}
	...
	//printk(KERN_INFO "%s: start block = %u, end block = %u\n", __func__, o_start, o_end);
	...
	while (o_start < o_end) {
		...
		if (ret) {
			//printk(KERN_INFO "%s: path not found\n", __func__);
			goto out;
		}
		ex = path[path->p_depth].p_ext;
		next_blk = ext4_ext_next_allocated_block(path);
		...
		if (cur_blk + cur_len - 1 < o_start) {
			if (next_blk == EXT_MAX_BLOCKS) {
				o_start = o_end;
				ret = -ENODATA;
				//printk(KERN_INFO "%s: next block = EXT_MAX_BLOCKS\n", __func__);
				goto out;
			}
			d_start += next_blk - o_start;
			o_start = next_blk;
			...
			/* Extent inside requested range ?*/
			if (cur_blk >= o_end) {
				//printk(KERN_INFO "%s: cur_blk >= o_end\n", __func__);
				goto out;
			}
		} else { /* in_range(o_start, o_blk, o_len) */
			cur_len += cur_blk - o_start;
		...
		*/
		...
		//LEDGER_START_TIMER(meta_extent_per_page_t, meta_extent_per_page_time);
		...
		//LEDGER_END_TIMER(meta_extent_per_page_t, meta_extent_per_page_time);
		
		if (ret < 0)
			break;
		o_start += cur_len;
		d_start += cur_len;
		//printk(KERN_INFO "%s: End of while. o_start = %u, o_end = %u\n", __func__, o_start, o_end);
	}
