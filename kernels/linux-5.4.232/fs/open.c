		       ...
		       loff_t count)
{
	...
	//printk(KERN_INFO "%s: off1 = %lld, off2 = %lld, addr = %lu, count = %lld\n",
	//       __func__, offset1, offset2, (unsigned long) start_addr, 
	//       count);
	...
	/*
	max_page_dirty = start_offset % PAGE_SIZE;
	if (max_page_dirty != 0)
		start_offset -= max_page_dirty;
	max_page_dirty = end_offset % PAGE_SIZE;
	if (end_offset != 0)
		end_offset += PAGE_SIZE - max_page_dirty;
	*/
	/*
	if (end_offset - start_offset >= 2097152) {
		printk(KERN_INFO "%s: unmapping start addr = %lu, length = %lu\n",
		       __func__, (unsigned long)start_addr + start_offset,
		       end_offset - start_offset);		
		mm = current->mm;
		ret_munmap = do_munmap(mm,
				       (unsigned long)start_addr + start_offset,
				       end_offset - start_offset, &uf);
	...
	*/
	...
	//printk(KERN_INFO "%s: calling ext4_dynamic_remap with offset1 = %lld, offset2 = %lld, count = %lld\n", __func__, offset1, offset2, count);
	...
	/*
	if (count != 0) {
		ret += vfs_write(file1, start_addr + offset2, count, &offset1);
		if (ret < len) {
			goto out;
		}			
	}
		...
		loff_t, count)
{
	...
	if (f1.file && f2.file) {
		//printk(KERN_INFO "%s: fd1 = %d, fd2 = %d, off1 = %lld, off2 = %lld, count = %lld\n",
		//       __func__, fd1, fd2, offset1, offset2, count);
